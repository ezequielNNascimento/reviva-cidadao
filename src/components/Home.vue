<!-- eslint-disable vue/multi-word-component-names -->
<script setup lang="ts">
import ProgressBar from './ui/progressBar/ProgressBar.vue'
import Button from './ui/button/Button.vue'

// Ícones do lucide-vue-next
import {
  User,
  Megaphone,
  Sprout,
  CalendarDays,
  Map,
  HelpCircle
} from 'lucide-vue-next'



</script>

<template>
  <section class="bg-gray-100 min-h-screen w-full flex flex-col items-center py-5">
  <div class="bg-gradient-to-r from-blue-800 to-blue-900 text-white p-6 rounded-lg w-full max-w-md mx-auto shadow-lg">
    <!-- Saudação -->
    <h1 class="text-xl font-bold">
      Olá, <span class="underline decoration-blue-400">USUÁRIO</span>
    </h1>
    <p class="mt-2 text-sm">
      Seu selo de contribuição é:
      <span class="font-extrabold tracking-wide ml-1">
        Guardião da Cidade
      </span>
    </p>

    <!-- Barras de Progresso -->
    <div class="mt-4 flex gap-2">
      <ProgressBar color="green" />
      <ProgressBar color="amber" />
      <ProgressBar color="blue" />
    </div>
  </div>

  <!-- Seção de Atividades -->
  <div class="mt-6 w-full max-w-md mx-auto">
    <h2 class="text-lg font-bold text-blue-900 mb-3">Atividades</h2>

    <div class="flex flex-col gap-3">
      <Button class="flex items-center justify-start gap-3 bg-white text-black hover:bg-gray-200">
        <User class="w-5 h-5" />
        Minha Conta
      </Button>

      <router-link
        to="/Denuncia"
        class="flex items-center justify-start gap-3 bg-white text-black hover:bg-gray-200 p-2 rounded"
>
        <Megaphone class="w-5 h-5" />
        Denúncias
      </router-link>

      <Button class="flex items-center justify-start gap-3 bg-white text-black hover:bg-gray-200">
        <Sprout class="w-5 h-5" />
        Trabalho Voluntário
      </Button>

      <Button class="flex items-center justify-start gap-3 bg-white text-black hover:bg-gray-200">
        <CalendarDays class="w-5 h-5" />
        Eventos
      </Button>

      <Button class="flex items-center justify-start gap-3 bg-white text-black hover:bg-gray-200">
        <Map class="w-5 h-5" />
        Mapa de Calor
      </Button>

      <Button class="flex items-center justify-start gap-3 bg-white text-black hover:bg-gray-200">
        <HelpCircle class="w-5 h-5" />
        Como usar o Aplicativo
      </Button>
    </div>
  </div>
  </section>
</template>
